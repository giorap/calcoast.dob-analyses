...
date <- date[-c(which(date %in% as.Date(c("2011/02/23", "2011/04/16", "2011/04/03", "2011/04/14"))))]
...
urls <- paste0("https://thredds.cencoos.org/thredds/ncss/UCSC.nc?var=", 
               rep(c("angle", "h", "ubarrot", "vbarrot", "zeta", "rho", "salt", "urot", "vrot", "w"), each = length(date)), 
              "&disableLLSubset=on&disableProjSubset=on&horizStride=1&time=",
              date,
              "T09%3A00%3A00Z&vertCoord=&addLatLon=true&accept=netcdf"
)
parLapply(cl, urls, purrr::safely(function(url){
...
roms_files <- list.files("X://grapacciuolo/ROMS_download/ROMS_ncdf/", full.names = TRUE)
sapply(roms_files, FUN = function(eachPath){
  file.rename(from=eachPath, to=gsub("T09%3A00%3A00Z&vertCoord=&addLatLon=true&accept=netcdf", "", gsub("&disableLLSubset=on&disableProjSubset=on&horizStride=1", "", eachPath)))
})
